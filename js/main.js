...
define(['canvasControls', 'html', 'tubeEvents', 'parseJson', 'animationControls', 'cameras', 'terrain'], 
    ...
    (function() {
        ...
        setupRenderer();
        Cameras.initCameras(parent);
        ParseJson.init();
        Html.addContent(renderer); // add options for dropdown 
        stats = Html.addStatsBar();
        Html.addHelpManoeuvreList(ParseJson.getManoeuvreArray());
        CanvasControls.init(renderer, Cameras.getStandardCamera()); // setup event listeners for canvas movements
        AnimationControls.initContolEvents(renderer, Cameras, parent); // setup listeners for changes to controls
        animate();
    })();
     ...
     */
    function render() {
        if (!AnimationControls.getIsPaused())
            renderPlane();

        scene.rotation.y += (CanvasControls.getLatestTargetRotationX() - scene.rotation.y) * 0.6;
        scene.rotation.x += (CanvasControls.getLatestTargetRotationY() - scene.rotation.x) * 0.6;
        scene.position.x += (CanvasControls.getLatestMoveX() - scene.position.x);
        scene.position.z += (CanvasControls.getLatestMoveZ() - scene.position.z);
        renderer.render(scene, Cameras.getIsOnboard() === true ? Cameras.getSplineCamera() : Cameras.getStandardCamera());
    }
    ...
    function renderPlane() {
        ...
        var scale = AnimationControls.getScale();
        var speed = AnimationControls.getAnimationSpeed();
        var time = Date.now();
        var looptime = speed * 1000;
        var t = (time % looptime) / looptime;
        var tube = AnimationControls.getTube();
        ...
        var splineCamera = Cameras.getSplineCamera();
        Cameras.setSplineCameraPosition(pos);
        Cameras.setCameraEyePosition(pos);
        var cameraEye = Cameras.getCameraEye();
        ...
        Cameras.setSplineCameraLookAt(lookAt, normal);
        Cameras.setRenderCamerasRotation();
    }
});
